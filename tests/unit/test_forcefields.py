...
def test_list_water_models_in_folder_reads_an_arbitrary_ff_folder(tmp_path):
    custom_ff = tmp_path / "somewhere" / "oplsaam.ff"
    custom_ff.mkdir(parents=True)
    (custom_ff / "watermodels.dat").write_text("tip3p TIP3P TIP 3-point\n")

    models = list_water_models_in_folder(custom_ff)

    assert [m.name for m in models] == ["tip3p"]


def test_list_water_models_in_folder_returns_empty_when_no_file(tmp_path):
    custom_ff = tmp_path / "bare.ff"
    custom_ff.mkdir()

    assert list_water_models_in_folder(custom_ff) == []
